...
_IBSheet.CommonOptions = {
      ...
      Menu: {
        ...
        "OnSave": function (item, data) {
          ...
          if (item) {
            ...
            switch (item.Name) {
              case '컬럼 감추기':
                if ((sheet.id.indexOf('pivotSheet_sheet') == 0) && item.Owner.Row[col+'Span'] > 1) {
                  const rowSpan = item.Owner.Row[col+'Span'];
                  const colIdx = sheet.getColIndex(col, true);
                  for (var i = 0; i < rowSpan; i++) {
                    const mergedCol = sheet.getColByIndex(colIdx + i, true);
  ...
  Events: {
    "onKeyDown": function (evtParam) {
      ...
      if (evt.sheet.MultiRecord) return;
      if (evt.menu && evt.row.Kind == "Header" && (evt.menu.Items[0] && !evt.menu.Items[0].Items) && (!evt.sheet.PivotSheet || evt.sheet.id.indexOf("pivotSheet_sheet") == -1)){
        evt.menu.Items.unshift(setContextMenu(evt.sheet),
        {
          "Name": "*-"
        });
      } else if (evt.menu && evt.row.Kind == "Header" && (evt.menu.Items[0] && !evt.menu.Items[0].Items) && evt.sheet.PivotSheet) {
        var found = false;
        for (var i = 0; i < evt.menu.Items.length; i++) {
          if (evt.menu.Items[i].Name == "컬럼 감추기") {
            found = true;
            break;
          }
        }
      ...
      if (evt.menu && evt.row.Kind == "Header" && evt.menu.Items[0].Items){
        var sheet = evt.sheet.Dialog ? evt.sheet.Dialog : evt.sheet;
        var items = evt.menu.Items[0].Items;
        if(items) setItemBool(items, sheet);
      }
    }
  }
};
